...
/obj/item/gun/attackby(obj/item/I, mob/user, params)
	if(user.a_intent == INTENT_HARM)
		...
		if(!can_bayonet || !K.bayonet || bayonet)
			return ..()
		if(!user.transferItemToLoc(I, src))
			return
		...
		bayonet = K
		var/state = "bayonet"
		//if(bayonet.icon_state in icon_states('icons/obj/guns/bayonets.dmi'))
...
/obj/item/gun/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(!user.canUseTopic(src, BE_CLOSE, FALSE, NO_TK))
		return
	//if((can_flashlight && gun_light) && (can_bayonet && bayonet))
	if(FALSE)//not_actual
	...
	else if(gun_light && can_flashlight)
...
/obj/item/gun/proc/update_gunlight()
	if(gun_light)
		...
		var/state = "flight[gun_light.on? "_on":""]"
		//if(gun_light.icon_state in icon_states('icons/obj/guns/flashlights.dmi'))
