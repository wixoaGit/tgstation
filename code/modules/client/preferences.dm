...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		if (0)
			...
			//if(istype(pref_species, /datum/species/ethereal))
		...
		if (1)
			...
			for (var/i in GLOB.special_roles)
				if(is_banned_from(user.ckey, i))
					...
					//if(ispath(GLOB.special_roles[i]) && CONFIG_GET(flag/use_age_restriction_for_jobs))
					...
					if(days_remaining)
						...
						dat += "<b>Be [capitalize(i)]:</b> <font color=red> (IN [days_remaining] DAYS)</font><br>"//not_actual
					else
	...
	var/datum/browser/popup = new(user, "preferences_browser_NEW", "<div align='center'>Character Setup</div>", 640, 770)//not_actual
...
/datum/preferences/proc/process_link(mob/user, list/href_list)
	...
	if(href_list["preference"] == "job")
		...
		//			return
		//		for(var/V in SSquirks.quirk_blacklist)
		//			var/list/L = V
		//			for(var/Q in all_quirks)
		//				if((quirk in L) && (Q in L) && !(Q == quirk))
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				//			features["mcolor"] = pref_species.default_color
				//		else if((MUTCOLORS_PARTSONLY in pref_species.species_traits) || ReadHSV(temp_hsv)[3] >= ReadHSV("#7F7F7F")[3])
