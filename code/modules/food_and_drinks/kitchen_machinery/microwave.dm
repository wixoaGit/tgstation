...
/obj/machinery/microwave/Initialize()
	...
	soundloop = new /datum/looping_sound/microwave(list(src), FALSE)//not_actual
...
/obj/machinery/microwave/attackby(obj/item/O, mob/user, params)
	...
	if(broken > 0)
		if(broken == 2 && O.tool_behaviour == TOOL_WIRECUTTER)
			...
			if(O.use_tool(src, user, 20))
				...
				broken = 1
		else if(broken == 1 && O.tool_behaviour == TOOL_WELDER)
			user.visible_message("[user] starts to fix part of \the [src].", "<span class='notice'>You start to fix part of \the [src]...</span>")
			if(O.use_tool(src, user, 20))
...
/obj/machinery/microwave/proc/loop(type, time, wait = max(12 - 2 * efficiency, 2))
	if(stat & (NOPOWER|BROKEN))
		if(MICROWAVE_PRE)
			pre_fail()
		return
