		...
		if (43)
			...
			for(var/obj/machinery/computer/monitor/pMon in GLOB.machines)
				if(pMon.stat & (NOPOWER | BROKEN))
					continue
				if(pda_turf.z != pMon.z)
					continue
				if(pMon.is_secret_monitor)
					continue
				powercount++
				powermonitors += pMon
...
/obj/item/cartridge/proc/bot_control()
	...
	//	else
	//		menu += "<BR>\[<A href='byond://?src=[REF(src)];op=patroloff'>Stop Patrol</A>\] "
	//		menu += "\[<A href='byond://?src=[REF(src)];op=patrolon'>Start Patrol</A>\] "
	//		menu += "\[<A href='byond://?src=[REF(src)];op=summon'>Summon Bot</A>\]<BR>"
	...
	//		if(!Bot.on || Bot.z != zlevel || Bot.remote_disabled || !(bot_access_flags & Bot.bot_type))
	//			continue
